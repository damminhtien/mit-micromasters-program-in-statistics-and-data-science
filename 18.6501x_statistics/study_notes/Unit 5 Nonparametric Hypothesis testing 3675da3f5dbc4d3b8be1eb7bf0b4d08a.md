...
### Exercise 1: Determine the appropriate tests
...
The multinomial distribution can be represented by a random vector $\mathbf N\in \mathbb {Z}^K$ containing the number of observations $N^{(i)}$ of outcome $i=1,\dots,K$. Note that $\sum_{i=1}^K N^{(i)}=n$. The **multinomial PMF** for all $\mathbf n$ such that $\sum_{i=1}^K n^{(i)}=n$, $n^{(i)}\ge0$, and $n^{(i)}\in\mathbb Z$ is given by

$$
\displaystyle p_{\mathbf N}\left(N^{(1)}=n^{(1)},\dots,N^{(K)}=n^{(K)}\right) = \frac{n!}{n^{(1)}!n^{(2)}!\cdots n^{(K)}!}\prod_{i=1}^K p_i^{n^{(i)}}.
$$
...
The first choice is a valid goodness-of-fit test while the second choice is not. Our aim is to test the candidate distribution against all other possible distributions. The first choice addresses that question directly.
...
Since $K = 3$ and $d = 1$, we know that the limiting distribution of $T_ n$ is $\chi _2^2$. Thus, the asymptotic level is the value $\tau$ such that

| $\lim _{n \to \infty } P( T_ n > \tau ) = P( Z > \tau ) = 0.05$ |  |
| --- | --- |

where $Z \sim \chi _{2}^2$. Hence, $\tau$ should be chosen to be $5.991$ (from the given table).
...
|  | $\displaystyle T_ n = 1000$ | $\displaystyle \cdot \bigg( \frac{\left(\frac{339}{1000} - \binom {3}{0} (0.2977^0) (0.7023)^{3 - 0} \right)^2}{\binom {3}{0} (0.2977^0) (0.7023)^{3 - 0} } + \frac{\left(\frac{455}{1000} - \binom {3}{1} (0.2977^1) (0.7023)^{3 - 1} \right)^2}{\binom {3}{1} (0.2977^1) (0.7023)^{3 - 1} } +$ |  |  |
| --- | --- | --- | --- | --- |
|  |  | $\displaystyle \frac{\left(\frac{180}{1000} - \binom {3}{2} (0.2977^2) (0.7023)^{3 - 2} \right)^2}{\binom {3}{2} (0.2977^2) (0.7023)^{3 - 2} } + \frac{\left(\frac{26}{1000} - \binom {3}{3} (0.2977^3) (0.7023)^{3 - 3} \right)^2}{\binom {3}{3} (0.2977^3) (0.7023)^{3 - 3} } \bigg)$ |  |  |
|  |  | $\displaystyle \approx 0.8829$ |  |  |
...
According to the golden rule of p-values, since $0.6431 > 0.05$, we should

**fail to reject**

the null hypothesis that $X_1, \ldots , X_{1000}$ are distributed as $\text {Bin}(3, \theta )$ for some value of the parameter $\theta$.
